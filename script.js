/* ═══════════════════════════════════════════════════════════════
   FAKE NEWS DETECTOR — INTERACTIVE ENGINE (FIXED)
...
(function() {
    ...
    const state = { currentPage: 0, totalPages: 6, charts: {} };
        ...
        if (timeCtx && !state.charts.time) {
            state.charts.time = new Chart(timeCtx, {
                ...
                data: {
                    ...
                    datasets: [{ data: [0.45, 2.8, 1.2, 3.5], backgroundColor: ['#3b82f6', '#10b981', '#f59e0b', '#ef4444'], borderColor: '#1e293b', borderWidth: 2 }]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
                    plugins: { legend: { position: 'right', labels: { color: '#94a3b8', font: { family: 'Inter', size: 10 }, boxWidth: 12 } } },
                    cutout: '65%'
                }
            });
        }
