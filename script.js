...
(function() {
    ...
    const state = { currentSection: 0, charts: {} };
    const $ = (sel) => document.querySelector(sel);
    const $$ = (sel) => document.querySelectorAll(sel);
    const sections = ['home', 'features', 'models', 'demo', 'metrics', 'about'];
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    // Init charts when metrics section is visible
                    if (id === 'metrics') setTimeout(initCharts, 200);
                }
            });
    ...
    function initNavClicks() {
        ...
        // Floating nav arrows
        $('#prevPage')?.addEventListener('click', () => scrollToSection(state.currentSection - 1));
        $('#nextPage')?.addEventListener('click', () => scrollToSection(state.currentSection + 1));
